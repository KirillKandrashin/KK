...
def create_brand(cursor):
    try:
        ...
        print('Ошибка! Эта таблица уже существует')
...
def parse_date(url,cursor,website_id,name,id_):
    ...
    if r.status_code==200:
        ...
        cursor.execute(f"insert into cinema_halls values ({id_},2,{website_id},'{name}','{address}',NULL,NULL)") 
        return dates
    else:
        print('Страница не найдена')
    ...
    if r.status_code==200:
        ...
        for film in films:
            ...
            if not(name in [el[0] for el in cursor.execute('select name from cinemas').fetchall()]):
                ...
                cursor.execute(f'insert into cinemas values({id1},NULL,"{name}","{duration}","{genres}")')
                id1+=1
            ...
            for time in times:
                ...
                cursor.execute(f"insert into sessions values({id2}, {cinema_id}, {id_-1},'{date}','{time1}')")
                id2+=1
    else:
        print('Страница не найдена')
    return id1,id2 
...
for i in range(10):
    try:
        ...
        print(f'Что-то пошло не так раз')
